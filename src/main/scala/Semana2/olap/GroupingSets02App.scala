...
object GroupingSets02App extends App with SparkSessionWrapper {
  ...
  spark.conf.set("spark.sql.shuffle.partitions", "10") // Viene por defecto 200, pero es un valor muy alto.
  ...
  val usarExistentes = false // false si no existe y crea los datos, true si existe.
  val BucketSize = 4
  ...
  if (!usarExistentes) {
    ...
    df.write.partitionBy("Category").bucketBy(BucketSize, BucketColumn).saveAsTable(TablaPorNombreYCategoria) // El particionado afecta a nivel del directorio donde se guardaran los datos, si tenemos elemento sin categoria se guardaria a Null
  } else {
