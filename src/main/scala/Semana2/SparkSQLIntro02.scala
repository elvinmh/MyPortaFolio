...
object SparkSQLIntro02 extends App {
  ...
  val df = spark.read
    ...
    .json(IotDataGenerator.IotJsonDataPath)  // IotDataGenerator es un fichero, que genera la data
  ...
  println(df.count()) // Recordar no abusar de la función count, se suele utilizar en desarrollo o si es necesario tener la cantidad de datos

  df.isEmpty  //Para saber si el df tiene datos

  import scala.language.postfixOps

  println(df count) // Esto es lo mismo que println(df.count()), pero para eso hubo que importar import scala.language.postfixOps
  ...
  df.filter(col("temperature") > 5).show() //Compatible con PySpark
  ...
  val dfSinTemperatura: DataFrame = df.drop("temperature") // Drop me da un nuevo DataFrame con la columna borrada
