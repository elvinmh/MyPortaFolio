...
object EncodingAvroApp extends App {
  ...
  val personV1SchemaFile = "src/main/scala/semana2/avro/person_v1.avsc"  //Ruta del archivo

  //Este esquema siempre sera igual
  // Read schema file
  val schemaSource = Source.fromFile(personV1SchemaFile) //Leemos el fichero y lo guardamos en esquema source, que no da el string que tiene el fichero
  val schemaV1 = try schemaSource.mkString finally schemaSource.close()

  // Parse the schema

  val parserV1 = new Schema.Parser  //Valida que esquema esté correcto
  val avroSchemaV1: Schema = parserV1.parse(schemaV1)
  val schemaStrV1 = avroSchemaV1.toString // Se convierte en sistema avro
  ...
  // Escribimos el DataFrame en formato Avro, esto es opcional
  df.write
    .format("avro")
...
//System.exit(0)
  ...
  val personV2SchemaFile = "src/main/scala/semana2/avro/person_v2.avsc"
  ...
  val dfReadV1 = spark.read
    .format("avro")
    .option("avroSchema", schemaStrV2) // Aquí leerá los datos de v1, como el v2.
    .load("data/avro/person_v1")
