...
object EjemplosSparkRDD extends App {
  ...
  val filtrarSiPar: Int => Boolean = (x: Int) => x % 2 == 0  // Para Fortalecer el inliner ponemos la función en una variable.
 ...
 // System.exit(0)
  ...
  dfWordCount.show() // Esto nos da como resultado lo mismo que lo anterior pero sin usar tuplas
  //System.exit(0)
  ...
  def funcionCompleja(x: Int): Int = {
    ...
    if (x % 2 == 0) x * x  // si es par
    else if (x % 3 == 0)   // Si es multiplo de 3
      unaFuncionCustomizada(x * x * x)
    else   //Si no cumple con ninguna condición
...
// El binario lo devuelve en texto plano
